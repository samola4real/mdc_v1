...
class ProductionRouteSafetyTests(SimpleTestCase):
    ...
    def test_canonical_v1_health_available(self):
        response = self.client.get("/api/v1/health", secure=True)

        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_canonical_v1_catalog_filters_available(self):
        response = self.client.get("/api/v1/catalog/filters", secure=True)
    ...
    def test_canonical_v1_service_discovery_search_available(self, fuseki):
